    ...
    <div className="flex flex-col w-full">
      ...
      {!isLoading && data && (
        <div className="absolute top-full z-50 bg-white dark:bg-default-50 shadow-lg w-[50%] border-default-500 border-2">
            <Listbox onAction={onAction} items={data} className="flex flex-col overflow-y-auto">
              {item =>(
                 <ListboxItem href={`/questions/${item.id}`}
                  startContent={
                   ...
                   <div>
                      <div className="font-semibold">{item.title}</div>
                       <div className="text-xs opacity-60 line-clamp-2">{item.content}</div>
                   </div>
                 </ListboxItem>
                )}
            </Listbox>
